...
class sudoku():
    def __init__(self):
        ...
        self.count=0                        #to escape backtracking recursive
    ...
    def __write(self,event):
        ...
        if event.keysym=='Return':
            ...
            self.canvas1.delete(self.num[self.row][self.col])
            self.num[self.row][self.col]=self.canvas1.create_text(x,y,text=int(event.char),font=40,tag='char')
    ...
    def __printing(self):
        ...
        for i in range(9):
            for j in range(9):
                ...
                self.num[i][j]=self.canvas1.create_text(x,y,text=self.mainarr[i][j],font=40,tag='char')
                self.canvas1.update()
    ################################ Backtrack  ###################################
